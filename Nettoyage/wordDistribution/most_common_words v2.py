...
for i, text in enumerate(text_list):
    ...
    for word in words:
        if word in most_common_words:
            ...
            if score_list[i] in most_common_words[word]["score"]:
...
sorted_words = sorted(most_common_words.items(), key=lambda x: x[1]["count"], reverse=True)
...
plt.bar(range(n_words), [x[1]["count"] for x in sorted_words[:n_words]], align='center')
